...
class DeepResearchSkill:
    ...
    async def generate_search_queries(self, query: str, num_queries: int = 3) -> List[Dict[str, str]]:
        ...
        messages = [
            {"role": "system", "content": "你是一名生成搜索查询的专业级金融分析员"},
            {"role": "user",
             "content": f"鉴于以下提示，生成{num_queries} 个独特的搜索查询，以全面研究该主题。对于每个查询，提供一个研究目标。格式为 “查询：< 查询内容 >”，后面跟着 “目标：< 目标内容 >”，每个查询对都按此格式：: {query}"}
        ]
    ...
    async def generate_research_plan(self, query: str, num_questions: int = 3) -> List[str]:
        ...
        messages = [
            {"role": "system", "content": "你是一名专业的金融分析人员。你的任务是分析原始查询和搜索结果，然后提出有针对性的问题，从不同方面和时间段来分析该主题。"},
            {"role": "user",
...
根据这些结果、原始查询以及当前时间，生成  {num_questions} 个独特的问题。每个问题都应从不同方面或时间段分析该主题，并考虑到直至{current_time}的信息.

每个问题单独占一行，以 “问题：” 开头进行格式编排 '"""}
        ]
    ...
    async def process_research_results(self, query: str, context: str, num_learnings: int = 3) -> Dict[str, List[str]]:
        ...
        messages = [
            {"role": "system", "content": "你是一名分析搜索结果的专业级金融分析人员"},
            {"role": "user",
             "content": f"针对 '{query}' 给出以下分析结果, 请提炼关键要点并提出后续问题。对于每个要点，如果有可用的来源网址，请附上引用。每个要点的格式为'Learning [source_url]: <insight>' 并且每个问题格式为 'Question: <question>':\n\n{context}"}
        ]
    ...
    async def deep_research(
        ...
        for result in results:
            ...
            if depth > 1:
                ...
                next_query = f"""
                之前的研究目标: {result['researchGoal']}
                接下来的问题: {' '.join(result['followUpQuestions'])}
                """

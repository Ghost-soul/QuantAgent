...
class EditorAgent:
    ...
    def _create_planning_prompt(self, initial_research: str, include_human_feedback: bool,
        ...
        return [
            {
                "role": "system",
                "content": "你是一位金融分析报告编辑人。你的目标是监督整个金融分析项目。"
                           "从开始到完成，你的主要任务是规划文章的各个部分。"
                           "文章的第一部分只进行对应的行业分析及宏观背景分析。"
                           "文章的第二部分只包括近期A股市场投资者的情绪分析。"
                          "文章的第三部分只包括使用技术分析方法分析股票交易数据并给出未来可能的走势。"
                           "文章的第四部分总结前三部分内容被给出具体的投资建议。（格式为 1.xx股票 对应的分析结果，2.xx股票 对应的分析结果）按推荐度从高到低进行排序。 "
                           "请基于初步分析摘要的布局.\n ",
            },
            
            {
                ...
                "content": f"""今天是{today}
                   ...
                   {feedback_instruction}
                   \n你的任务是根据上述分析摘要报告生成各章节的标题大纲,这里必须紧扣'{initial_research}'。
                   你只需返回一个 JSON，结构如下
                   '{{title: string research title, date: today's date, 
                   sections: ['section header 1', 'section header 2', 'section header 3','summary']}}'
                   其中section header 1 只进行对应的行业分析及宏观背景分析,包括宏观经济环境，政府政策，技术创新等方面。
                   section header 2 只进行近期A股市场投资者的情绪分析。
                   section header 3 只通过使用技术分析方法分析股票交易数据并给出未来可能的走势。
                   summary 总结前三部分内容并给出具体的投资建议。（格式为 1.xx股票 对应的分析结果；2.xx股票 对应的分析结果；3.xx股票 对应的分析结果...）按推荐度从高到低进行排序。
                   """,
            },
        ]
    ...
    def _create_workflow(self) -> StateGraph:
        ...
        #elif source == "local":
         #   workflow.add_node("researcher", agents["research"].run_local_research)
        #else:
        workflow.add_node("researcher", agents["research"].run_hybrid_research)
